### This is an example how to trigger the manal failover
...
if jenkins_master_1
    ...
    `vagrant ssh jenkins1 -c "sudo service jenkins stop" 2>&1`
    `vagrant ssh jenkins2 -c "sudo chown -R jenkins:jenkins /var/lib/jenkins/" 2>&1`
    `vagrant ssh jenkins2 -c "sudo service jenkins start" 2>&1`
    
end
if jenkins_master_2
    puts "   - Master 2 is currently active"
    puts "   - Making master 1 active"
    `vagrant ssh jenkins2 -c "sudo service jenkins stop" 2>&1`
    `vagrant ssh jenkins1 -c "sudo chown -R jenkins:jenkins /var/lib/jenkins/" 2>&1`
    `vagrant ssh jenkins1 -c "sudo service jenkins start" 2>&1`
